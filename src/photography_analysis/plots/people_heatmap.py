...
def plot_heatmap_plotly(
    ...
    name_grid = np.repeat(np.array(labels).reshape(-1, 1), len(month_labels), axis=1)
    month_str_grid = np.tile(month_labels.strftime("%Y-%m"), (len(order), 1))
    customdata = np.dstack((raw_counts, name_grid, month_str_grid))
    ...
    fig.add_trace(go.Heatmap(
        ...
        hovertemplate=(
            "%{customdata[1]}<br>"
            "%{customdata[2]}<br>"
            "%{customdata[0]:.0f} photos<extra></extra>"
        ),
        colorscale="Viridis",
        xgap=0.5,
        ygap=0.5,
    ...
    fig.update_xaxes(
        range=[month_labels[0], month_labels[-1]],
        tickmode="array",
